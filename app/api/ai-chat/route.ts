...
${clipped}`;
  ...
  let positionsInfo = "";
  if (context?.positions && typeof context.positions === "string" && context.positions.length > 10) {
    positionsInfo = `

## 📒 ユーザーの現在のオープン建玉（方向と理由は唯一の正・絶対に矛盾するな）
以下はユーザーが実際に建てているポジションだ。方向（現物／信用買い／信用売り）と建玉理由を必ず尊重せよ。
ユーザーが空売りしている建玉に対して「なぜ売っているのか」と問い返すのは重大な誤り。継続/利確/損切りのいずれかで助言せよ。
